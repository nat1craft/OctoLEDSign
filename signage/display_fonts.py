...
class font_proportional(proportional):
    def __init__(self,font,id=-1):
        super().__init__(font=font)
        self.id = id

class font_tolerant(tolerant):
    def __init__(self,font,id=-1, missing="_"):
        super().__init__(font=font,missing=missing)
        self.id = id

#: Bit patterns for CUSTOM_TINY_FONT,
#: Originally based on: TINY_FONT
#: source: http://www.dafont.com/tiny.font,
#: transposed by Richard Hull.
#:
#: NOTE: Only contains some characters.
...
CUSTOM_TINY_FONT = [
    ...
    [0x00, 0x2A, 0x14, 0x00],  # '}'
    [0x04, 0x04, 0x0C, 0x00],  # '~'
    [0x00, 0x14, 0x2A, 0x00],  # 0x7F - wavy
    [0x04, 0x3E, 0x04, 0x00],  # 0x80 - arrow up
    [0x10, 0x3E, 0x10, 0x00],  # 0x81 - arrow down
    [0x78, 0x28, 0x18, 0x00],  # 0x82 - PM
    [0x70, 0x28, 0x70, 0x00],  # 0x83 - AM
    ...
    [0x01, 0x00, 0x00, 0x00],  # 0xB0 - degree
    [0x0F, 0x05, 0x00, 0x00],  # 0xB1 - degree F
    [0x07, 0x05, 0x00, 0x00],  # 0xB2 - degree C
